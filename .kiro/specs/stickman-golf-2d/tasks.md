...
- [x] 9. Input_Controller — keyboard only, relative stepping and the absolute path
  ...
  - **Measured, both conditions met in a real browser.** On a fresh load `document.activeElement` is `BODY`, so nothing is focused; two ArrowLeft presses with no pointer interaction moved the aim 0 to 10 and tilted the indicator, with `window.scrollY` still 0. Writing 135 into the focused `overlay-aim-input` set the aim to 135, and ArrowUp then stepped power 50 to 55 while that field still held focus, leaving the aim at 135 — the field's own spinner never fired. 27 further browser checks over R7.7, R7.8, R7.13, R7.15, R7.16, R7.20 through R7.26 all pass.
  - **Two findings worth carrying into tasks 12 and 13.** First, a Playwright-based driver **cannot** write non-numeric text into an `input[type=number]`; it refuses with "Cannot type text into input[type=number]". R7.22's non-numeric branch is therefore unreachable through the driver and was verified by direct assignment instead, and no Verification_Flow can trigger it. Second, a native number input reports `value` as the empty string whenever its text is unparseable, so R7.22's empty and non-numeric cases arrive indistinguishably; both restore, so nothing turns on telling them apart.
  - **One deliberate deviation from R7.26, recorded rather than hidden.** A number input that currently holds focus is not rewritten from the held value, because snapping on every keystroke makes the field impossible to type into — entering 135 would pass through 0, then 5, and land on 55. What holds instead: the read-only `overlay-aim-angle` and `overlay-power` fields always show the held value; each number input shows it too whenever that input is not being edited; the text is reconciled on blur, on `change` and on Enter; a relative arrow step rewrites the field even while focused; and an unparseable write restores the text immediately, focused or not, exactly as R7.22 requires. An agent that writes a field and reads the read-only field back — which is what both flows do — never sees a disagreement.
  - Estimate: **60 min**
  - _Requirements: R7.1–R7.8, R7.11–R7.26, R8.2_

- [x] 10. Debug_Overlay — the frozen DOM contract
  ...
  - **Measured.** 31 identifiers declared; 30 present and non-empty on first paint, with `overlay-result` correctly absent because the phase reads `IN_PROGRESS`. 126 browser assertions pass: presence, non-emptiness, no surrounding whitespace, set membership for all 11 enumerated fields, bare-decimal-integer format for the rest, nothing exposed beyond the declared set, no field inside the canvas, and both number inputs reading back their read-only counterparts. A further 79 assertions in `verification/tools/check-overlay-contract.ts` compare the coded contract against an **independent transcription** of Requirement 9, so a typo'd identifier or a value dropped from a frozen set fails under Node with no browser needed.
  - **The contract data was moved to `shared/overlay-contract.ts`.** R9.16 freezes it for the life of the project, and a contract readable only from inside a browser is a contract nothing can check. The module holds the identifiers, the frozen value sets, R9.28's fixed values and R9.15's number format as pure data with no DOM reference, so `client/src/overlay.ts` renders it and a Node tool asserts against it from the same declaration. `client/src/overlay.ts` re-exports it, so consumers still read the contract from one import.
  - **`overlay-result` is the one field not rendered in every phase**, and R15.34 is the reason it can be. R9.17 declares its value set as exactly `P1`, `P2`, `TIE`, `VOID` with no `NONE` member, so before the Match completes there is no truthful in-set value it could hold; rendering a placeholder would put an out-of-set value on screen and fail every flow under R15.17. The ten per-Arena Stroke fields, which R9.19 also scopes to `MATCH_COMPLETE`, **are** rendered throughout, because a Stroke count is a non-negative integer in every phase.
  - Estimate: **60 min**
  - _Requirements: R9.1–R9.16, R9.19–R9.28, R10.9, R10.15, R5.1, R5.2, R5.3_

- [x] 11. Local Arena 1 loop — Status_Token state machine, shot accounting, start-arena selector
  ...
  - **Measured, all three met.** 29 browser assertions and 68 Node assertions pass. Four ArrowUp presses and a space press, with no pointer interaction anywhere, take Arena 1 from `BALL_AT_REST` through `BALL_MOVING` to `IN_HOLE` with `overlay-p1-hole-out` reading `HOLED_OUT_BY_CAPTURE` in one Stroke; `?arena=2` starts in Arena 2 with the anomaly count at 0; `?arena=banana` and `?arena=3` both start in Arena 1 with the anomaly count at 1 and the anomaly naming the refused value. The start-arena selector is a query parameter, which is the only thing a browser navigation can carry.
  - **The Match state machine is a pure module** (`client/src/game.ts`) with no browser interface, so `verification/tools/check-match.ts` plays whole Arenas under Node: the Status_Token edges, one Stroke per Shot, the `MAX_STROKES_PER_ARENA` cap latching `HOLED_OUT_BY_STROKE_CAP` without ever reaching `IN_HOLE`, `ALREADY_HOLED_OUT` and `BALL_NOT_AT_REST` rejections, the out-of-bounds reset to the pre-shot position, and every one of the 2,736 grid Shots reaching a terminal token with no anomaly. With Requirement 18 descoped this is the only automated check the state machine gets, which is exactly why it is not locked inside a browser.
  - **Two defects the acceptance check caught, both fixed.** First, `overlay-p1-arena-N-strokes` read 0 for the Arena being played, because the retained per-Arena record is only written when an Arena completes (R13.3). R13.4's note is explicit that the current Arena's Strokes belong in the per-Arena field and not in the total, so a separate display projection now merges the live count over the retained record. Second, and more consequential for the harness: the overlay only rendered on the animation frame, so for about 17 milliseconds after a space press the DOM still read `BALL_AT_REST`, and a flow that fired and immediately polled would exit its poll on the stale value and waste an Agent_Step. The same lag put `overlay-aim-angle` behind `overlay-aim-input` and broke R7.26 for one frame. The overlay is now rendered synchronously on three transitions as well as on the frame — a Shot result, a Shot completing, and any aim or power change — so no reader can observe a stale field.
  - **`IN_HOLE` and `OUT_OF_BOUNDS` are held until the Player acts**, rather than collapsing to `BALL_AT_REST` on the step they are set. That is the local reading of R5.9 and R5.10's "until the Game_Server publishes the Active_Player designation for the next Shot": with no Game_Server, the designation is published when the Player asks for the next turn. It matters because a token held for one Simulation_Step would be missed by almost every poll at `STATUS_POLL_INTERVAL_MILLISECONDS`, which would make `OUT_OF_BOUNDS` dead in the delivered scope — precisely what D-18 moved Arena 2's open edge to prevent. It costs no Agent_Step, because the acknowledgement is folded into the same space press that fires the next Shot.
  - Estimate: **45 min**
